...
function cfg = setDirectionsRepeated(cfg)
    ...
    static_directions=[-1 -1 -1 -1 -1 -1 -1 -1 ]; 
%     static_directions=[-1 -2 -1 -2 -1 -2 -1 -2 ]; %%%%%
...
%                 repeatShuffleConditions(CONDITION1_DIRECTIONS, NB_REPEATS_BASE_VECTOR);
            ...
            directionsA(CONDITON2_INDEX(iMotionBlock), :) = [180 90 180 90 180 90 180 90]; %changed from the above line to create sequential stimuli
    ...
    directionsB= changem(directionsA,[-1, -2, 0, 180, 270, 90], [-2, -1, 180, 0, 90, 270]);%creates another matrix B with values replaced for opposite directions
